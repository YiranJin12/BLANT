...
vector<int> getConnectedNodes(int nodeIndex, const vector<vector<int>>& adjMatrix) {
    vector<int> connectedNodes;
    // Check the row corresponding to the single nodeIndex
    for (int i = 0; i < (int)adjMatrix[nodeIndex].size(); ++i) {
        if (adjMatrix[nodeIndex][i] == 1) {
            connectedNodes.push_back(i);
        }
    }
...
int main() {
    ...
    string filenameGraph1 = "rsrc/human.el";  // read from file(may need use .el file)
    string filenameGraph2 = "rsrc/yeast.el";
    string filenameSeed = "rsrc/seed.txt";  // Replace with your actual file path
    string filenameSim = "rsrc/yeast-human.txt";
    ...
    for (const auto& simPair : similarityPairs) {
        if (get<0>(simPair) == nodePair.first && get<1>(simPair) == nodePair.second) {
            ...
            cout << "(" << nodePair.first << ", " << nodePair.second
                 << ") with similarity " << get<2>(simPair) << endl;
            skiplist.insertElement(get<2>(simPair),nodePair.first, nodePair.second);
        }
    }
}
    ...
    double key;
    int first, second;
    tie(key, first, second) = tup;

    if (key != -1) {  // meaning we actually got a valid tuple
        // interpret 'key' as the similarity value
    
        cout << "Popped node => key: " << key << ", VertexA: " << first << ", VertexB: " << second;
        alignmentList.push_back({ first, second });
        cout << "[DEBUG] Using maxNode: (" << first << ", " << second
         << ") with value=" << key << endl;
    ...
    cout << "[DEBUG] skiplist is empty, no candidate pairs.\n";
    }
